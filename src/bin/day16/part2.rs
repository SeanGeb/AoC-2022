...
fn partition_dfs<'a>(
    ...
    for partition_bitvec in 0..=bitvec_max {
        ...
        best_score = best_score.max(score_human + score_elephant);

        if partition_bitvec % 256 == 0 {
            println!("Progress: {partition_bitvec}/{bitvec_max}");
        }
    }
